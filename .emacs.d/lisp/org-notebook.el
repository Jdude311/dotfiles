...
;;;###autoload
(defun refresh-images (process event)
  (org-redisplay-inline-images))

(defun org-notebook-insert-image-from-default ()
  ...
  (let ((org-notebook-image-filepath
    ...
    (shell-command (format "cp ./img/default.png %s" org-notebook-image-filepath))
    (set-process-sentinel (start-process "org-notebook-drawing" nil org-notebook-drawing-program org-notebook-image-filepath) 'refresh-images)))

(defun org-notebook-insert-image-blank ()
  "Insert an image with auto-completion for the next image name and open the drawing program"
  (interactive)
  (unless (file-directory-p "./img") (make-directory "./img"))
  (let ((org-notebook-image-filepath
	  ...
	  (read-from-minibuffer
           ...
           (format "img%d.png"
                   (+ (string-to-number
                      ...
                      1))))))
    (insert "[[" org-notebook-image-filepath "]]")
    (shell-command (format "touch %s" org-notebook-image-filepath))
    (set-process-sentinel (start-process "org-notebook-drawing" nil org-notebook-drawing-program org-notebook-image-filepath) 'refresh-images)))

(defun xournalpp-convert-to-image ()
  (set-process-sentinel (start-process "xournalpp -i" org-notebook-image-filepath) 'org-redisplay-inline-images))
  ...
  (let ((org-notebook-image-filepath
	 (concat
	  ...
	  (read-file-name
           "PDF: "))))
   (insert "[[" org-notebook-image-filepath "]]")
   (set-process-sentinel (start-process "xournalpp") 'xournalpp-convert-to-image)))
